...
class translate_form extends \moodleform {
    ...
    public function definition() {
        ...
        if (!empty($config->deeplfree_api_key)) {
            $apis['deeplfree'] = get_string('selectdeeplfree', 'local_bftranslate');
        }
        ...
        if (count($apis) > 0) {
            $mform->addElement(
                'select',
                'selectapi',
                get_string('selectapi', 'local_bftranslate'),
                $apis
            );
        ...
        $mform->addElement(
            'select',
            'targetlang',
            get_string('selectlanguage', 'local_bftranslate'),
            $targetlanguages
        );
        ...
        $mform->addElement(
            'advcheckbox',
            'showexisting',
            get_string('showexisting', 'local_bftranslate'),
            get_string('showexisting_desc', 'local_bftranslate')
        );
